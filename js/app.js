 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        parallax_Parallax.Each = class {
            ...
            parameters(el, parameters) {
                if (parameters.axis === "v") {
                    const translateY = (parameters.direction * (this.value / parameters.coefficient)).toFixed(2);
                    const transformedValue = (this.initialOffset - translateY).toFixed(2);
                    el.style.transform = `translate3D(0, ${transformedValue}px, 0) ${parameters.additionalProperties}`;
                } else if (parameters.axis === "h") {
                    const translateX = (parameters.direction * (this.value / parameters.coefficient)).toFixed(2);
                    const transformedValue = (this.initialOffset - translateX).toFixed(2);
                    el.style.transform = `translate3D(${transformedValue}px, 0, 0) ${parameters.additionalProperties}`;
                }
            }
        };
        ...
        const observer = new MutationObserver((mutationsList => {
            for (const mutation of mutationsList) if (mutation.attributeName === "class") if (heroBlock.classList.contains("_watcher-view")) header.classList.add("_backNone"); else header.classList.remove("_backNone");
        }));
        observer.observe(heroBlock, {
            attributes: true
        });
        const title = document.querySelector(".title-block__title_main");
